...
Rules for tagging:
- If the majority of sources are positive on a player → use positive tags like Breakout, Bounce Back, Rookie Riser, Workhorse, WR1 Role, Target Hog
- When an analyst explicitly identifies a player as a league winner, must-have, or high-conviction season-long target, apply a tag in the format "[FirstName] LW" using that analyst's last name as it appears in the source text — for example "Boone LW", "Berry LW", "Henry LW". If multiple analysts make this call for the same player, apply one such tag per analyst. If no analyst name is identifiable, use "Analyst LW" as the fallback.
...
- NEVER apply contradictory tags to the same player (e.g. never apply both a "[Name] LW" tag and Analyst Fade)
...
Valid tags: Workhorse, Committee Back, WR1 Role, Target Hog, Depth Only, Handcuff, Boom/Bust, Analyst Fade, Injury Risk, Breakout, Bounce Back, Rookie Riser, Aging Concern, Situation Change, Bust Risk, and "[Analyst Last Name] LW" (e.g. "Boone LW") for league-winner calls — use "Analyst LW" if no analyst name is identifiable`,
