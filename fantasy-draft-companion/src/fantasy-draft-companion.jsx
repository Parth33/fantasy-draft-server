...
export default function App() {
  ...
  const MeterBar = ({label, value, colorFn}) => (
    <div style={{marginBottom:5}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
        ...
        <span style={{fontSize:10,fontWeight:500,color:colorFn(value)}}>{value}</span>
      </div>
  ...
  const PlayerRow = ({p}) => {
    ...
    return (
      <div onClick={()=>setSelected(p)} style={{display:"grid",gridTemplateColumns:"24px 28px 1fr 38px 38px 68px 80px 52px 38px 70px",gap:4,alignItems:"center",padding:"5px 8px",borderRadius:6,background:selected?.id===p.id?"var(--bg-row-sel)":isRec?"var(--bg-row-rec)":"var(--bg-row)",border:`1px solid ${selected?.id===p.id?"var(--border-accent)":isRec?"#1a3a1a":"var(--border)"}`,cursor:"pointer",fontSize:11,marginBottom:2,transition:"background 0.1s"}}>
        <span style={{color:"var(--text-muted)",textAlign:"right"}}>{p.rank}</span>
        <span style={{fontSize:9,fontWeight:700,padding:"2px 4px",borderRadius:4,background:POS_COLORS[p.pos]||"#555",color:"#fff",textAlign:"center",letterSpacing:"0.03em"}}>{p.pos}</span>
        ...
        <span style={{color:"var(--text-muted)",fontSize:9}}>Bye {p.bye}</span>
        {ol?<span style={{fontSize:9,color:olineColor(ol.label)}}>Line: {ol.label}</span>:<span/>}
        {d?<span style={{fontSize:9,color:defColor(d.label)}}>Def #{d.rank} {d.label}</span>:<span/>}
        {s?<span style={{fontSize:9,color:sosColor(s.e)}}>SOS {s.e}</span>:<span/>}
        <span style={{fontSize:9,fontWeight:500,color:safetyColor(p.safety)}}>{p.safety}</span>
        <div style={{display:"flex",gap:3}}>
          <button onClick={e=>{e.stopPropagation();markDrafted(p,true);}} style={{fontSize:9,padding:"2px 5px",borderRadius:"var(--radius)",border:"0.5px solid var(--border-accent)",background:"var(--bg-accent)",color:"var(--text-accent)",cursor:"pointer"}}>Mine</button>
          <button onClick={e=>{e.stopPropagation();markDrafted(p,false);}} style={{fontSize:9,padding:"2px 5px",borderRadius:"var(--radius)",border:"0.5px solid var(--border)",background:"transparent",color:"var(--text-muted)",cursor:"pointer"}}>Gone</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{fontFamily:"system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",color:"var(--text-primary)",background:"var(--bg-app)",minHeight:"100vh"}}>
      ...
      `}</style>
      <h2 className="sr-only">NFL Fantasy Draft Companion</h2>

      {/* Top bar */}
      <div style={{background:"var(--bg-header)",borderBottom:`1px solid var(--border)`,padding:"8px 14px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <span style={{fontWeight:600,fontSize:14,color:"var(--text-accent)",letterSpacing:"-0.01em"}}>⚡ Draft Companion</span>
        <div style={{display:"flex",gap:4}}>
          {LEAGUES.map((l,i)=>(
            <button key={i} onClick={()=>setLeague(i)} style={{fontSize:11,padding:"4px 10px",borderRadius:6,border:`1px solid ${league===i?"var(--border-accent)":"var(--border)"}`,background:league===i?"var(--bg-accent-soft)":"transparent",color:league===i?"var(--text-accent)":"var(--text-secondary)",cursor:"pointer",fontWeight:league===i?500:400}}>{l.name} ({l.teams}t)</button>
          ))}
        </div>
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          {isMyTurn&&<span style={{fontSize:11,padding:"3px 8px",borderRadius:"var(--radius)",background:"var(--bg-success)",color:"var(--text-success)",fontWeight:500}}>Your pick</span>}
          <span style={{fontSize:11,color:"var(--text-secondary)"}}>R{round} P{pick}</span>
          <span style={{fontSize:10,color:"var(--text-muted)"}}>Pos</span>
          <input type="number" min={1} max={teams} value={draftPos} onChange={e=>setDraftPos(Number(e.target.value))} style={{width:36,fontSize:11,textAlign:"center"}}/>
          <button onClick={()=>setTheme(t=>t==="dark"?"light":"dark")} style={{fontSize:11,padding:"3px 9px",borderRadius:6,border:`1px solid var(--border)`,background:"transparent",cursor:"pointer",color:"var(--text-secondary)",display:"flex",alignItems:"center",gap:4}}>
            {theme==="dark"?"☀ Light":"🌙 Dark"}
          </button>
          <button onClick={undoLast} style={{fontSize:10,padding:"3px 8px",borderRadius:6,border:`1px solid var(--border)`,background:"transparent",cursor:"pointer",color:"var(--text-secondary)"}}>Undo</button>
        </div>
      </div>

      {/* Tabs */}
      <div style={{display:"flex",borderBottom:`1px solid var(--border)`,background:"var(--bg-header)",padding:"0 14px"}}>
        {[{k:"board",l:"Draft Board"},{k:"roster",l:`Roster (${roster.length})`},{k:"odef",l:"O-Line / Defense"},{k:"camp",l:"Camp Intel"},{k:"mock",l:"Mock Draft"}].map(t=>(
          <button key={t.k} onClick={()=>setActiveTab(t.k)} style={{padding:"9px 14px",fontSize:12,border:"none",borderBottom:activeTab===t.k?`2px solid var(--tab-active-border)`:"2px solid transparent",background:"transparent",cursor:"pointer",color:activeTab===t.k?"var(--text-accent)":"var(--text-secondary)",fontWeight:activeTab===t.k?600:400,letterSpacing:"0.01em"}}>{t.l}</button>
        ))}
        {campStatus==="done"&&<span style={{fontSize:10,padding:"2px 7px",borderRadius:"var(--radius)",background:"var(--bg-success)",color:"var(--text-success)",alignSelf:"center",marginLeft:4}}>Camp applied</span>}
      </div>

      <div style={{display:"flex",height:"calc(100vh - 94px)",overflow:"hidden"}}>

        {/* LEFT SIDEBAR: Scarcity + Tier counts */}
        <div style={{width:160,background:"var(--bg-sidebar)",borderRight:`1px solid var(--border)`,padding:10,overflow:"auto",flexShrink:0}}>
          <div style={{fontSize:10,fontWeight:500,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:8}}>Tier scarcity</div>

          {/* Run alert */}
          {runAlert&&(
            <div style={{background:"var(--bg-warning)",border:"0.5px solid var(--border-warning)",borderRadius:"var(--radius)",padding:"6px 8px",marginBottom:8,fontSize:10}}>
          ...
          {/* Tier drain alerts */}
          {tierAlerts.map((a,i)=>(
            <div key={i} style={{background:a.urgent?"var(--bg-danger)":"var(--bg-warning)",border:`0.5px solid ${a.urgent?"var(--border-danger)":"var(--border-warning)"}`,borderRadius:"var(--radius)",padding:"5px 7px",marginBottom:5,fontSize:10}}>
              <span style={{fontWeight:500,color:a.urgent?"var(--text-danger)":"var(--text-warning)"}}>{a.pos} T{a.tier}: {a.count} left</span>
              {a.urgent&&<div style={{color:"var(--text-danger)",fontSize:9}}>Act now</div>}
            </div>
          ))}
          ...
          {["QB","RB","WR","TE"].map(pos=>(
            <div key={pos} style={{marginBottom:10}}>
              <div style={{fontSize:10,fontWeight:500,color:"var(--text-secondary)",marginBottom:4}}>{pos}</div>
              {[1,2,3,4].map(tier=>{
                ...
                return (
                  <div key={tier} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"2px 0",borderBottom:"0.5px solid var(--border)"}}>
                    <span style={{fontSize:9,color:"var(--text-muted)"}}>T{tier}</span>
                    <span style={{fontSize:11,fontWeight:500,color}}>{count}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{marginTop:10,paddingTop:10,borderTop:"0.5px solid var(--border)"}}>
            <div style={{fontSize:10,fontWeight:500,color:"var(--text-muted)",marginBottom:6}}>My roster</div>
            ...
            {["QB","RB","WR","TE","K","DEF"].map(pos=>{
              ...
              return (
                <div key={pos} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",fontSize:10}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:12,background:"var(--bg-app)"}}>
          ...
          {activeTab==="board"&&(
            <>
              ...
              {isMyTurn&&recs.length>0&&(
                <div style={{marginBottom:10,background:"var(--bg-accent-soft)",border:`1px solid var(--border-accent)`,borderRadius:10,padding:"8px 12px"}}>
                  <div style={{fontSize:10,fontWeight:500,color:"var(--text-accent)",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.05em"}}>Your pick — top recommendations</div>
                  <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                    {recs.map((p,i)=>(
                      <div key={p.id} onClick={()=>setSelected(p)} style={{background:"var(--bg-card)",borderRadius:8,padding:"7px 10px",cursor:"pointer",border:`1px solid var(--border-accent)`,flex:1,minWidth:110}}>
                        <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:2}}>
                          <span style={{fontSize:9,color:"var(--text-muted)"}}>#{i+1}</span>
                          <span style={{fontSize:12,fontWeight:500}}>{p.name}</span>
                          <span style={{fontSize:9,padding:"1px 3px",borderRadius:3,background:TIER_COLORS[p.tier],color:"#fff"}}>{p.pos}</span>
                          {p.campAdj!==undefined&&p.campAdj!==0&&<span style={{fontSize:9,color:p.campAdj>0?"#0e9f6e":"#e03e3e"}}>{p.campAdj>0?"▲":"▼"}{Math.abs(p.campAdj)}</span>}
                        </div>
                        <div style={{fontSize:10,color:"var(--text-secondary)"}}>{getReason(p,roster,round)}</div>
                        <div style={{fontSize:9,color:safetyColor(p.safety),marginTop:2}}>{p.safety}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:8,alignItems:"center",flexWrap:"wrap"}}>
                <input type="text" placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:140,fontSize:11}}/>
                <div style={{display:"flex",gap:3}}>
                  {POSITIONS.map(pos=>(
                    <button key={pos} onClick={()=>setPosFilter(pos)} style={{fontSize:10,padding:"3px 6px",borderRadius:"var(--radius)",border:"0.5px solid var(--border-strong)",background:posFilter===pos?"var(--fill-accent)":"transparent",color:posFilter===pos?"var(--on-accent)":"var(--text-secondary)",cursor:"pointer"}}>{pos}</button>
                  ))}
                </div>
                <span style={{fontSize:10,color:"var(--text-muted)"}}>{available.length} available</span>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                <div>
                  <div style={{fontSize:11,fontWeight:500,color:"var(--text-secondary)",marginBottom:4,paddingBottom:4,borderBottom:"0.5px solid var(--border)"}}>Overall rankings</div>
                  <div style={{display:"grid",gridTemplateColumns:"24px 28px 1fr 38px 38px 68px 80px 52px 38px 70px",gap:4,padding:"0 8px 4px",fontSize:9,color:"var(--text-muted)",marginBottom:4}}>
                ...
                {/* RIGHT: Tier view */}
                <div>
                  <div style={{fontSize:11,fontWeight:500,color:"var(--text-secondary)",marginBottom:4,paddingBottom:4,borderBottom:"0.5px solid var(--border)"}}>By tier</div>
                  <div style={{display:"grid",gridTemplateColumns:"24px 28px 1fr 38px 38px 68px 80px 52px 38px 70px",gap:4,padding:"0 8px 4px",fontSize:9,color:"var(--text-muted)",marginBottom:4}}>
                    <span style={{textAlign:"right"}}>#</span><span>Pos</span><span>Player</span><span>Team</span><span>Bye</span><span>O-Line</span><span>Defense</span><span>SOS</span><span>Safety</span><span></span>
                  </div>
                  {tierOrder.map(key=>{
                    ...
                    return (
                      <div key={key} style={{marginBottom:8}}>
                        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                          <span style={{fontSize:9,fontWeight:700,color,textTransform:"uppercase",letterSpacing:"0.08em",padding:"1px 6px",borderRadius:4,background:`${color}22`,border:`1px solid ${color}44`}}>{pos} T{tier} — {label}</span>
          ...
          {activeTab==="roster"&&(
            <div>
              <div style={{marginBottom:10}}><span style={{fontSize:14,fontWeight:500}}>My roster</span><span style={{fontSize:12,color:"var(--text-secondary)",marginLeft:8}}>{roster.length}/15</span></div>
              {ROSTER_SLOTS.map((slot,i)=>{
                ...
                return (
                  <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",marginBottom:3,background:"var(--bg-card)",borderRadius:6,border:`1px solid var(--border)`}}>
                    <span style={{fontSize:10,fontWeight:500,color:"var(--text-muted)",width:36}}>{slot}</span>
                    {p?<>
                      <span style={{fontWeight:500,fontSize:12,flex:1}}>{p.name}</span>
                      {p.campAdj!==undefined&&p.campAdj!==0&&<span style={{fontSize:9,color:p.campAdj>0?"#0e9f6e":"#e03e3e"}}>{p.campAdj>0?"▲":"▼"}{Math.abs(p.campAdj)}</span>}
                      <span style={{fontSize:10,color:"var(--text-secondary)"}}>{p.team}</span>
                      <span style={{fontSize:9,padding:"1px 4px",borderRadius:3,background:TIER_COLORS[p.tier],color:"#fff"}}>T{p.tier}</span>
                      <span style={{fontSize:10,color:safetyColor(p.safety)}}>{p.safety}</span>
                      <span style={{fontSize:10,color:"var(--text-muted)"}}>Bye {p.bye}</span>
                      {SOS[p.team]&&<span style={{fontSize:9,color:sosColor(SOS[p.team].e)}}>SOS {SOS[p.team].e}</span>}
          ...
          {activeTab==="odef"&&(
            <div>
              <div style={{fontSize:13,fontWeight:500,marginBottom:4}}>O-line grades</div>
              <div style={{fontSize:11,color:"var(--text-secondary)",marginBottom:10}}>Run blocking for RBs. Pass blocking for WRs and QBs.</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:5,marginBottom:18}}>
                {Object.entries(OLINE).sort((a,b)=>b[1].run-a[1].run).map(([team,g])=>(
                  <div key={team} style={{background:"var(--bg-card)",border:`1px solid var(--border)`,borderRadius:6,padding:"7px 10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                      <span style={{fontWeight:500,fontSize:12}}>{team}</span>
                      <span style={{fontSize:9,padding:"1px 4px",borderRadius:3,background:olineColor(g.label),color:"#fff"}}>{g.label}</span>
                    </div>
                    <div style={{fontSize:10,color:"var(--text-secondary)"}}>Run {g.run} · Pass {g.pass}</div>
                  </div>
                ))}
              </div>
              <div style={{fontSize:13,fontWeight:500,marginBottom:4}}>Defensive rankings</div>
              <div style={{fontSize:11,color:"var(--text-secondary)",marginBottom:10}}>Weak defenses mean the opposing offense scores more to keep up. Good for your offensive players.</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:5}}>
                {Object.entries(DEF).sort((a,b)=>a[1].rank-b[1].rank).map(([team,d])=>(
                  <div key={team} style={{background:"var(--bg-card)",border:`1px solid var(--border)`,borderRadius:6,padding:"7px 10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                      <span style={{fontWeight:500,fontSize:12}}>#{d.rank} {team}</span>
                      <span style={{fontSize:9,padding:"1px 4px",borderRadius:3,background:defColor(d.label),color:"#fff"}}>{d.label}</span>
                    </div>
                    <div style={{fontSize:10,color:"var(--text-secondary)"}}>{d.ppg} ppg allowed</div>
                    {d.shootout&&<div style={{fontSize:9,color:"#c27803",marginTop:2}}>Shootout likely</div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab==="camp"&&(
            <div>
              <div style={{fontSize:13,fontWeight:500,marginBottom:4}}>Camp intel</div>
              <div style={{fontSize:11,color:"var(--text-secondary)",marginBottom:12}}>Fetch the latest notes from Rotowire and NFL.com, or paste your own. The AI analyzes everything and adjusts rankings, risk, reward, and safety scores automatically.</div>

              <div style={{display:"flex",gap:8,marginBottom:10,alignItems:"center",flexWrap:"wrap"}}>
                <button onClick={fetchCampNews} disabled={fetchStatus==="loading"} style={{fontSize:12,padding:"7px 14px",borderRadius:6,border:`1px solid var(--border)`,background:"var(--bg-card)",cursor:fetchStatus==="loading"?"wait":"pointer",color:"var(--text-primary)",fontWeight:500}}>
                  {fetchStatus==="loading"?"Fetching...":"Fetch from Rotowire + NFL.com"}
                </button>
                {fetchStatus==="done"&&<span style={{fontSize:11,color:"var(--text-success)"}}>Fetched — review and hit Analyze</span>}
                {fetchStatus==="error"&&<span style={{fontSize:11,color:"var(--text-danger)"}}>Fetch failed — paste notes manually below</span>}
                {fetchStatus==="empty"&&<span style={{fontSize:11,color:"var(--text-warning)"}}>No results — paste notes manually</span>}

                <button onClick={fetchXHandles} style={{fontSize:11,padding:"5px 10px",borderRadius:"var(--radius)",border:"0.5px solid var(--border)",background:"transparent",cursor:"pointer",color:"var(--text-secondary)"}}>X handles to follow</button>
              </div>

              {xHandles&&(
                <div style={{marginBottom:12,background:"var(--surface-1)",border:"0.5px solid var(--border)",borderRadius:12,padding:12}}>
                  <div style={{fontSize:11,fontWeight:500,marginBottom:6}}>Key accounts to check on draft morning</div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {xHandles.leagueWide.map(h=>(
                      <div key={h.handle} style={{background:"var(--surface-2)",borderRadius:"var(--radius)",padding:"4px 8px",fontSize:10,border:"0.5px solid var(--border)"}}>
              ...
              <textarea value={campText} onChange={e=>setCampText(e.target.value)} placeholder="Paste training camp notes here, or hit Fetch above. One player per line works well." style={{width:"100%",minHeight:180,fontSize:11,fontFamily:"system-ui,sans-serif",lineHeight:1.6,resize:"vertical",boxSizing:"border-box",padding:10,borderRadius:6,border:`1px solid var(--border)`,background:"var(--bg-row)",color:"var(--text-primary)"}}/>

              <div style={{display:"flex",gap:8,marginTop:8,alignItems:"center"}}>
                <button onClick={runCampAnalysis} disabled={campStatus==="loading"||!campText.trim()} style={{fontSize:12,padding:"7px 16px",borderRadius:6,border:`2px solid var(--border-accent)`,background:"var(--bg-accent-soft)",color:"var(--text-accent)",cursor:campStatus==="loading"?"wait":"pointer",fontWeight:700}}>
                  {campStatus==="loading"?"Analyzing...":"Analyze and apply to rankings"}
                </button>
                {campStatus==="loading"&&<span style={{fontSize:10,color:"var(--text-muted)"}}>AI reading notes, adjusting rankings...</span>}
                {campLastRun&&<span style={{fontSize:10,color:"var(--text-muted)"}}>Last run {campLastRun.toLocaleTimeString()}</span>}
              </div>

              {campStatus==="done"&&Object.keys(campAdjs).length>0&&(
                <div style={{marginTop:16}}>
                  <div style={{fontSize:12,fontWeight:500,marginBottom:6}}>Adjustments applied to rankings</div>
                  <div style={{display:"flex",flexDirection:"column",gap:4}}>
                    {Object.entries(campAdjs).sort((a,b)=>Math.abs(b[1].adj)-Math.abs(a[1].adj)).map(([name,a])=>(
                      <div key={name} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:"var(--surface-1)",borderRadius:"var(--radius)",border:"0.5px solid var(--border)",fontSize:11}}>
                        <span style={{fontSize:14}}>{a.signal==="up"?"▲":a.signal==="down"?"▼":"—"}</span>
                        ...
                        <span style={{fontSize:9,padding:"1px 5px",borderRadius:3,background:safetyColor(a.safety||"Solid"),color:"#fff"}}>{a.safety}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {selected&&(
          <div style={{width:230,background:"var(--bg-sidebar)",borderLeft:`1px solid var(--border)`,padding:12,overflow:"auto",flexShrink:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              <button onClick={()=>setSelected(null)} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",fontSize:15,padding:0}}>✕</button>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:12}}>
              {[{l:"Rank",v:`#${selected.rank}`},{l:"ADP",v:selected.adp.toFixed(1)},{l:"Tier",v:`T${selected.tier}`},{l:"Half PPR",v:`${selected.ppg}`}].map(m=>(
                <div key={m.l} style={{background:"var(--bg-row)",borderRadius:6,padding:"7px 9px",border:`1px solid var(--border)`}}>
                  ...
                  <div style={{fontSize:14,fontWeight:500,marginTop:1}}>{m.v}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{background:"var(--bg-row)",borderRadius:6,padding:"10px 12px",marginBottom:8,border:`1px solid var(--border)`}}>
              <div style={{fontSize:10,fontWeight:700,color:"var(--text-secondary)",marginBottom:8,textTransform:"uppercase",letterSpacing:"0.06em"}}>Safety meter</div>
            ...
            {selected.campSummary&&(
              <div style={{background:selected.campAdj>0?"var(--bg-success)":selected.campAdj<0?"var(--bg-danger)":"var(--surface-1)",borderRadius:"var(--radius)",padding:"8px 10px",marginBottom:8}}>
                <div style={{fontSize:9,color:"var(--text-muted)",marginBottom:3}}>Camp analysis</div>
                <div style={{fontSize:11,fontWeight:500,color:selected.campAdj>0?"var(--text-success)":selected.campAdj<0?"var(--text-danger)":"var(--text-primary)",marginBottom:2}}>
                  {selected.campAdj>0?`▲ Up ${selected.campAdj} spots`:selected.campAdj<0?`▼ Down ${Math.abs(selected.campAdj)} spots`:"Neutral"}
                </div>
                <div style={{fontSize:10,color:"var(--text-secondary)"}}>{selected.campSummary}</div>
              </div>
            )}
            ...
            {(selected.pos==="RB"||selected.pos==="WR"||selected.pos==="QB")&&(()=>{
              ...
              return (
                <div style={{background:"var(--bg-row)",borderRadius:6,padding:"9px 11px",marginBottom:8,border:`1px solid var(--border)`}}>
                  <div style={{fontSize:9,fontWeight:600,color:"var(--text-secondary)",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>O-line ({selected.pos==="RB"?"run":"pass"} block)</div>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                    <span style={{fontSize:15,fontWeight:500,color:olineColor(ol.label)}}>{ol.score}</span>
                    <span style={{fontSize:10,color:olineColor(ol.label)}}>{ol.label}</span>
                  </div>
            ...
            {DEF[selected.team]&&(()=>{
              ...
              return (
                <div style={{background:"var(--surface-1)",borderRadius:"var(--radius)",padding:"8px 10px",marginBottom:8}}>
                  <div style={{fontSize:9,fontWeight:600,color:"var(--text-secondary)",marginBottom:3,textTransform:"uppercase",letterSpacing:"0.05em"}}>Team defense</div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:12,fontWeight:500}}>#{d.rank} overall</span>
                    <span style={{fontSize:9,padding:"1px 4px",borderRadius:3,background:defColor(d.label),color:"#fff"}}>{d.label}</span>
                  </div>
                  <div style={{fontSize:10,color:"var(--text-secondary)",marginTop:2}}>{d.ppg} ppg allowed</div>
                  {d.shootout&&<div style={{fontSize:10,color:"#c27803",marginTop:2}}>Weak D = more offensive volume</div>}
                </div>
              );
            })()}
            ...
            {SOS[selected.team]&&(()=>{
              ...
              return (
                <div style={{background:"var(--surface-1)",borderRadius:"var(--radius)",padding:"8px 10px",marginBottom:10}}>
                  <div style={{fontSize:9,fontWeight:600,color:"var(--text-secondary)",marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Schedule</div>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <div><div style={{fontSize:9,color:"var(--text-muted)"}}>Wk 1-6</div><div style={{fontSize:15,fontWeight:500,color:sosColor(s.e)}}>{s.e}</div></div>
                    <div style={{textAlign:"right"}}><div style={{fontSize:9,color:"var(--text-muted)"}}>Full season</div><div style={{fontSize:15,fontWeight:500,color:sosColor(s.f)}}>{s.f}</div></div>
                  </div>
                </div>
              );
            })()}
            ...
            <div style={{display:"flex",gap:4}}>
              <button onClick={()=>markDrafted(selected,true)} style={{flex:1,fontSize:11,padding:"7px 0",borderRadius:6,border:`2px solid var(--border-accent)`,background:"var(--bg-accent-soft)",color:"var(--text-accent)",cursor:"pointer",fontWeight:700}}>✓ My pick</button>
              <button onClick={()=>markDrafted(selected,false)} style={{flex:1,fontSize:11,padding:"7px 0",borderRadius:6,border:`1px solid var(--border)`,background:"transparent",color:"var(--text-muted)",cursor:"pointer"}}>✕ Gone</button>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
        <span style={{fontSize:13,fontWeight:500}}>Mock draft</span>
        <span style={{fontSize:11,color:"var(--text-secondary)"}}>R{round} P{pick} · Pos {draftPos}</span>
        {myTurn&&!done&&<span style={{fontSize:10,padding:"2px 6px",borderRadius:"var(--radius)",background:"var(--bg-success)",color:"var(--text-success)"}}>Your pick</span>}
        <button onClick={reset} style={{marginLeft:"auto",fontSize:11,padding:"3px 9px",borderRadius:"var(--radius)",border:"0.5px solid var(--border)",background:"transparent",cursor:"pointer",color:"var(--text-secondary)"}}>Reset</button>
      </div>
      {done?<div style={{textAlign:"center",padding:28,color:"var(--text-secondary)",fontSize:12}}>Mock complete. {roster.length} players drafted. Reset to run again.</div>
      :myTurn?(
        <div style={{marginBottom:12}}>
          <div style={{fontSize:11,color:"var(--text-secondary)",marginBottom:5}}>Your turn:</div>
          {recs.map((p,i)=>(
            <div key={p.id} onClick={()=>advance(p,true)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"var(--bg-accent)",border:"0.5px solid var(--border-accent)",borderRadius:"var(--radius)",cursor:"pointer",marginBottom:4}}>
              ...
              <span style={{fontWeight:500,fontSize:12,flex:1}}>{p.name}</span>
              {p.campAdj!==undefined&&p.campAdj!==0&&<span style={{fontSize:9,color:p.campAdj>0?"#0e9f6e":"#e03e3e"}}>{p.campAdj>0?"▲":"▼"}{Math.abs(p.campAdj)}</span>}
              <span style={{fontSize:9,padding:"1px 4px",borderRadius:3,background:TIER_COLORS[p.tier],color:"#fff"}}>{p.pos}</span>
              <span style={{fontSize:10,color:"var(--text-secondary)"}}>{p.team}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{marginBottom:12}}>
          <button onClick={sim} style={{fontSize:12,padding:"7px 14px",borderRadius:"var(--radius)",border:"0.5px solid var(--border-strong)",background:"transparent",cursor:"pointer"}}>Simulate next pick</button>
        </div>
      )}
      <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:10}}>
        {roster.map(p=>(
          <span key={p.id} style={{fontSize:10,padding:"2px 7px",borderRadius:"var(--radius)",background:"var(--surface-1)",border:"0.5px solid var(--border)"}}>{p.name.split(" ").pop()} <span style={{color:"var(--text-muted)"}}>{p.pos}</span></span>
        ))}
      </div>
      <div style={{fontSize:11,fontWeight:500,color:"var(--text-secondary)",marginBottom:4}}>Pick log</div>
      <div style={{maxHeight:160,overflow:"auto",background:"var(--surface-1)",borderRadius:"var(--radius)",border:"0.5px solid var(--border)"}}>
        {log.map((l,i)=>(
          <div key={i} style={{display:"flex",gap:7,padding:"3px 8px",borderBottom:"0.5px solid var(--border)",fontSize:10,background:l.team==="You"?"var(--bg-accent)":"transparent"}}>
        ...
        {!log.length&&<div style={{padding:"10px 8px",fontSize:10,color:"var(--text-muted)",textAlign:"center"}}>No picks yet</div>}
      </div>
    </div>
  );
}
